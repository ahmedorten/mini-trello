...
function mockCustomers(overrides: {
  ...
  const store = reactive({
    ...
    filters: { page: 1, pageSize: 20, search: '', status: '', type: '', city: '' },
    load: vi.fn(async () => {}),
    setSearch: vi.fn(),
...
describe('CustomersView', () => {
  ...
  it('changing the status filter calls the store setter', async () => {
    ...
    const wrapper = await mountWithRouter();
    const selects = wrapper.findAll('.customers__filters select');
  ...
  it('typing in the search box calls setSearch once after the debounce', async () => {
    ...
    const wrapper = await mountWithRouter();
    await wrapper.find('.customers__filters input[type="search"]').setValue('orten');
